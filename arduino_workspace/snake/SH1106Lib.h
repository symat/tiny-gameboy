...
void SH1106Lib::initialize()
{
  ...
  // this was set to 0x80, setting the oscillator frequency to f_osc-20% instead of the default f_osc
  //sendCommand(SH1106_SETDISPLAYCLOCKDIV);            // 0xD5
  //sendCommand(0x80);  
  // the suggested ratio 0x80
  
  // no need to init, this is the default
  ...
  // I don't get this... CHARGEPUMP is not 0x8D but 0x30..0x33
  // 0x8D is an invalid command, and then 0x10 sets the column address to 0 ?
