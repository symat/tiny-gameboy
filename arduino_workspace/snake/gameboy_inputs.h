...
uint8_t button1 = 0;
uint8_t button2 = 0;
int8_t joystick_x = 0;  // -2 LEFT  ...  2 RIGHT
int8_t joystick_y = 0;  // -2 UP  ...  2 DOWN
#if BUTTON_3_ENABLED
uint8_t button3 = 0;
#endif


#ifdef RANDOM_SEED_BY_JOYSTICK
uint8_t random_seed_defined = 0;   // will be updated with the first joystick move
#endif
...
int8_t map_to_logical_joystick_value(int8_t scaled) {
  if(scaled > 47) return 2;
  if(scaled > 1) return 1;
  if(scaled > -2) return 0;
  if(scaled > -48) return -1;
  return -2;
}
...
void read_all_inputs() { 
  read_inputs(PB3, &adc3_btn1_y);
  read_inputs(PB4, &adc4_btn2_x);
#if BUTTON_3_ENABLED 
  read_inputs(PB5, &adc5_btn3);
#endif
  update_state();
