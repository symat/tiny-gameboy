...
uint8_t still_playing() {
  return current_tone_freqs == 0 ? 0 : 1;
}
...
void update_tones() {
  ...
  if(note_pause) {
    ...
    uint16_t actualFreq = pgm_read_word(current_tone_freqs + note_next);
    if(actualFreq == NOTE_PAUSE) {
      noTone(SPEAKER_PIN);
    } else {
      if(enable_sound) tone(SPEAKER_PIN, actualFreq);
    }
...
void play_tones(uint16_t* tone_freqs, uint8_t* tone_lengths, uint16_t _number_of_notes) {
